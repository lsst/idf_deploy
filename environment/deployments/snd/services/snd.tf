# Frontend GCS Bucket
resource "google_storage_bucket" "snd_web" {
  name                        = "snd-web-${var.environment}"
  location                    = "US"
  uniform_bucket_level_access = true
  force_destroy               = var.snd_web_bucket_force_destroy
...
resource "google_storage_bucket" "snd_api" {
  ...
  location                    = "US"
  uniform_bucket_level_access = true
  force_destroy               = var.snd_api_bucket_force_destroy
}

# Public read access
resource "google_storage_bucket_iam_member" "snd_web_public" {
  bucket = google_storage_bucket.snd_web.name
  role   = "roles/storage.objectViewer"
  member = "allUsers"
}

resource "google_storage_bucket_iam_member" "snd_api_public" {
...
resource "google_compute_backend_bucket" "snd_web" {
  name        = "snd-web-origin"
  bucket_name = google_storage_bucket.snd_web.name
  enable_cdn  = true
...
resource "google_compute_backend_bucket" "snd_api" {
  name        = "snd-api-origin"
  bucket_name = google_storage_bucket.snd_api.name
  enable_cdn  = true
...
# Global Static IP
resource "google_compute_global_address" "snd_lb_ip" {
  name = "snd-lb-ip"
}
...
resource "google_compute_url_map" "snd" {
  name            = "snd-url-map"
  default_service = google_compute_backend_bucket.snd_web.id

  host_rule {
    hosts        = [local.snd_hostname]
    path_matcher = "main"
  }
